         ...
         while (count < maxIterations)
         {
            ...
            range (i, COUNT(scene, 2, indices) / 3)
            {
               ...
               auto handleCylinder = [&](vec3 p, vec3 q) { // infinite cylinder from p to q
                  ...
                  vec3 pCrossA = cross(p, a);
                  vec3 vCrossA = cross(distance, a);
                  float numeratorPart = dot(vCrossA, pCrossA);
                  float discriminant = dot(vCrossA, vCrossA) * square(player.innerRadius) - square(dot(p, vCrossA));
                  float denom = dot(vCrossA, vCrossA);
                  ...
                  auto handleCollision = [&](float time)
                  {
                     ...
                     float signedDistance = dot(a, pos - p);
